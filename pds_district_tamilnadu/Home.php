	...
	function setSelectedValue1(obj_value,valueToSet) {
		...
		for (var i = 0; i < obj.options.length; i++) {
			if (obj.options[i].text== valueToSet) {
                ...
                <ul class="breadcrumb">
                    <li><a href="#">Home</a></li>
                    <li class="active">Tamil Nadu Route  Optimisation For PDS</li>
                </ul>
                <!-- END BREADCRUMB -->
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="panel panel-default">
								<div class="panel-heading">
                                    <h3 class="panel-title">Tamil Nadu Route Optimisation For PDS District - <b><?php echo $district; ?></b> <div id="timer"> <b>Time Left &nbsp </b> <span id="countdown"></span></h3>
                                </div>
                            </div>
							<div class="row">
							...
							if(obj !== undefined && obj !== null){
								for (var datafield in obj) 
								{
									...
									if(reason_district.length>0){
										...
										var district_reason = "<td><select class='form-control' onchange='handleReasonChange(\"" + uniqueid_idreason + "\")' id='" + uniqueid_idreason + "' name='" + uniqueid_idreason + "' disabled><option value=''>Select</option><option value='Road not accessible'>Road not accessible</option><option value='Road repair going on'>Road repair going on</option><option value='Pertaining to Distance'>Pertaining to Distance</option><option value='Suggested distance is longer than existing'>Suggested distance is longer than existing</option><option value='Road in Poor conditions suggested'>Road in Poor conditions suggested</option><option value='Godown has limited capacity'>Godown has limited capacity</option><option value='Traffic Congestion Issues'>Traffic Congestion Issues</option><option value='Godown now become defunction and other admin reason'>Godown now become defunction and other admin reason</option><option value='Godown suggested are not PDS operative godowns (buffer godown)'>Godown suggested are not PDS operative godowns (buffer godown)</option><option value='Existing Godown to shop route is accessible easily than suggested one'>Existing Godown to shop route is accessible easily than suggested one</option><option value='Suggested Godown is in the same campus of existing Godown'>Suggested Godown is in the same campus of existing Godown</option><option value='Distance worked out wrongly by GoI'>Distance worked out wrongly by GoI</option></select></td>";
