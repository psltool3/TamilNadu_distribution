...
class Encryption
{
    ...
    public function decrypt($encryptedString, $key)
        {
            if (empty($encryptedString) || !is_string($encryptedString)) {
                return null;
            }

            $decoded = base64_decode($encryptedString, true);
            if ($decoded === false) {
                return null; // Not valid base64
            }

            $json = json_decode($decoded, true);
            if (!is_array($json) || !isset($json["salt"], $json["iv"], $json["ciphertext"], $json["iterations"])) {
