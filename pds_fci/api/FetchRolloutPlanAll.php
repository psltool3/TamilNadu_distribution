...
if ($result && $result->num_rows > 0 && $result_leg1 && $result_leg1->num_rows > 0) {
	...
	while($row = mysqli_fetch_assoc($result))
	{
		if($row['new_id_admin']!=null or $row['new_id_admin']!=""){
			$id = $row['new_id_admin'];
			$query_warehouse = "SELECT latitude,longitude,district,block FROM warehouse WHERE id='$id'";
			...
			if($numrows_warehouse!=0){
				...
				$row["from_block"] = $row_warehouse['block'];
			}
	...
	$tableData["WH_Used_Baseline"] = '78';
	$tableData["FPS_Used_Baseline"] = '90';
...
$resultarray["Reduction_OptimisedTotal"] = ((28.27-$resultarray["Average_Distance_OptimisedTotal"])/28.27)*100;
